...
/**
 * Execute a validated tool intent against the Hands layer.
 * 
 * Hardens against:
 ...
 * @public
 */
export async function gutCertifiedContract(
  envelope: CertifiedContractEnvelope,
  ...
  const onGlobalAbort = () => toolAbort.abort(globalKillSwitch.reason);
  globalKillSwitch.addEventListener("abort", onGlobalAbort, { once: true });

  try {
    // Dispatch to the Hands layer (handles Zod validation, timeouts, output fencing)
    const result = await catalogue.forwardIntent(intent, lease, toolAbort.signal);
    ...
    globalKillSwitch.removeEventListener("abort", onGlobalAbort);
  }
}
